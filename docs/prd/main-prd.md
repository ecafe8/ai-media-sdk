...
| v1.8.0 | 2026-07-31 | 纠正万相（Wan）异步结果形态为 `output.results[].url`（非 `output.choices[].message.content[].image`）。新增阿里云百炼视频异步契约（HappyHorse `t2v`/`i2v`）：提交 `video-generation/video-synthesis` + `X-DashScope-Async`，轮询 `GET /tasks/{task_id}`（与图像异步同一任务机制），结果 `output.video_url`。**SUB-006 视频生成提前评估**：因与图像异步共用核心任务契约（`TaskHandle<T>`/`GET /tasks/{task_id}`），Phase 4 一并交付核心模态无关异步契约（SUB-003）+ 视频异步首期接入（HappyHorse）。详见 `docs/prd/sub-provider-adapters/tech.md` v1.6.0 §4.5。 |
