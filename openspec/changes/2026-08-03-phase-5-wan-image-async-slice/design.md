...
The provider object serves both modalities; per-binding casts (`provider as unknown as ProviderAdapter<ImageContent[]>` when binding a Wan model, `ProviderAdapter<VideoContent[]>` when binding a HappyHorse video model) follow the existing Phase 4 pattern at `index.ts:160`. The `submit()` return type on the provider object itself is a union, but each binding sees only its own `TContent`.

_Why over per-modality adapter methods (approach B):_ avoids changing the core `ProviderAdapter<TContent>` contract and all existing adapters. Phase 4 already validated this family-branch pattern (HappyHorse t2v/i2v branch inside the same `submit`). The cast is localized to the provider factory, not leaked to callers.

### D4: Reuse the shared `getTask(taskId)` poll (no new poll code)

The `getTask(taskId)` built in Phase 4 (`GET /tasks/{task_id}` + status mapping `PENDING`→`pending`, `PROCESSING`/`RUNNING`→`running`, `SUCCEEDED`→`succeeded`, `FAILED`→`failed`, `CANCELED`→`cancelled`, `UNKNOWN`→`failed`) serves both video and image async without change. Only the result extraction in the submit's poll closure differs: image maps `output.results[].url` → `ImageContent[]` (potentially multi-element, unlike video's fixed 1).
...
`submit()` builds the `wan-image` body: `{ model, input: { prompt }, parameters: { size?, n?, seed? } }` where `parameters` fields are sourced from the public `ImageGenerationInput` (`size`/`n`) and Aliyun-native fields from `providerOptions.aliyun` (`seed` etc.). Headers: `X-DashScope-Async: enable`, `Authorization: Bearer {apiKey}`, `Content-Type: application/json`. The submit response `output.task_id` + `output.task_status` builds the `TaskHandle<ImageContent[]>`.

_Why:_ matches the live Wan image-async curl (`tech.md` §4.5). `prompt`, `n`, `size` are public; Aliyun-native fields travel under `providerOptions.aliyun` (consistent with the image `providerOptions.aliyun` boundary established in Phase 2 for Qwen).
