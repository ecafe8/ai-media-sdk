...
- 浏览器直连 Provider：客户端 executor 直接调用 `generateImage`/`editImage`/`submitImageTask`/`submitVideoTask`（含异步任务浏览器侧轮询），凭证由体验者在设置面板一次填写、持久化到 localStorage，仅发送至对应 Provider，不经过任何服务器。
- 图片输入支持本地文件：图生图参考图、i2v 首帧为单值"URL + 上传"混合控件；r2v 参考图由逗号分隔 textarea 改为有序卡片列表（上传/粘贴 URL/批量粘贴，顺序即 `[Image N]`）。文件以 `{ base64, mimeType }` 经 SDK `ImageContent` 通道发送（三家 adapter 均已支持）。
...
- `site-shell`: 纯前端站点的 Landing 页与 Playground 页外壳——SPA 路由、Landing 内容（特性/模型矩阵/隐私说明）、Playground 头部与模式切换、BYO 环境的文案与状态标识。
...
- 新增工作区：`apps/site/`（`package.json`、`vite.config.ts`、`tsconfig.json`、`eslint.config.js`、`index.html`、`src/`、`tests/`）；依赖 `@ai-media/sdk`、`@ai-media/provider-*`、`@workspace/ui`、`react-router`、`lucide-react`。
