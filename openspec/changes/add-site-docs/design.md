...
`apps/site` 是 Vite 7 + React Router 7 的纯前端 SPA，部署在 GitHub Pages 项目站点（base `/ai-media-sdk/`），已有 `VITE_SITE_BASE` 单一变量驱动资源 base 与 Router basename、`404.html` SPA 深链回退、`PageContainer` 统一宽度规则。Vite 配置已为 `@ai-media/*` 各包建立指向源码的 alias（文档组件可直接 import 各包注册表）。仓库现有 5 份 README（约 780 行）是内容改写基础；源码 TSDoc 注释完整度高，可支撑 API 参考生成。约束：不引入服务端、不改 SDK 运行时行为、遵守 monorepo UI 规则（shadcn 优先、不随意新增共享自定义组件）。
...
- **生成内容容错**：manifest 条目带 `generated?: boolean` 标记（API 参考条目为 true）。收集模块查表时，`generated` 条目对应文件缺失（本地 dev/test 未运行 `docs:generate`）渲染占位页（说明该页由源码生成、给出 `bun run docs:generate` 命令），不渲染空白页或抛错。站点 `dev`/`test` 脚本不强制依赖生成步骤，保持轻量；`build` 经 turbo 依赖保证生成先行。
...
- **前置验证（TypeScript 7 兼容性）**：仓库使用 `typescript@7.0.2`（tsgo 原生编译器），而 TypeDoc 依赖 TypeScript 的 JS 编译器 API，两者可能不兼容。实施第一步即验证 `typedoc` 能否在本仓库成功生成；不能则为 typedoc 单独锁定 `typescript@5.x`（根 devDependency + typedoc 专用 tsconfig/`--compilerLib` 指向 5.x），仍不可行则直接启用回退方案。
...
- 集成方式：生成的 `.md` 与手写 MDX 走同一条 Vite 管线（D1 的 include 已覆盖 `.md`），在 manifest 中为每个包登记 API 参考入口（`generated: true`，见 D2 容错）；生成文件间的相对 `.md` 链接由一个 remark 插件（站点内小工具）重写为站内 `/docs/...` 路由链接。
...
- 构建集成：`turbo.json` 新增 `docs:generate` 任务（根包任务），`apps/site` 的 `build` 任务经 `dependsOn: ["//docs:generate"]` 依赖它（turbo 2 以 `//` 前缀引用根包任务）；CI 的 `bunx turbo build --filter=site` 会自动带入该依赖，部署工作流无需新增步骤。本地直接 `bun run --cwd apps/site build`（绕过 turbo）不会触发生成，属预期行为（dev 场景由 D2 占位页兜底）。

### D6: 错误码与内容的单一事实源策略

- `SdkError` 错误码表只在 `error-handling.mdx` 维护；FAQ/Provider 页以链接引用。
- `UploaderError` 错误码表只在 `uploader.mdx` 维护；`file-upload.mdx` 以链接引用。两页分工：file-upload 为指南视角（上传工作流 + 与 `editImage` 的集成示例），uploader 为包参考视角（子路径导出、完整配置表、`UploadedFile` 形状、错误码表、配额限制）。
...
- 新增 `apps/site/tests/docs-manifest.test.ts`（bun test）：断言非生成类文档与 manifest 双向一致（清单有而文件缺失、或文件存在而清单缺失均失败）；`generated` 条目只校验清单侧存在性（文件缺失属未运行生成步骤，不视为错误）；清单分组非空、每篇非生成文档模块有默认导出。
- 既有 `postbuild.ts` 资源 base 校验继续覆盖构建产物；`site:preview` 人工核对深链。
- 不引入 React 渲染层测试框架（站点现状无此基建，避免扩大范围）。

## Risks / Trade-offs

- [TypeDoc 与 TypeScript 7（tsgo）不兼容，API 参考生成失败] → D5 前置验证任务先行；缓解依次为：为 typedoc 锁定 typescript@5.x 运行环境、切换回退方案（静态 HTML）。
...
- [13 篇内容写作量大、易跑偏] → tasks 按"每篇一页任务"拆分，每篇验收标准引用 spec 对应 Requirement，避免自由发挥。
