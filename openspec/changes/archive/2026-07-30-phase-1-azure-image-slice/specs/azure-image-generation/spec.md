...
The core `generateImage` function SHALL accept an image generation request carrying a provider-bound image model instance (produced by `provider.image(modelOrDeployment)`), validate public parameters against the instance capabilities, and dispatch to the instance's adapter `generate` instead of throwing `NOT_IMPLEMENTED`.
