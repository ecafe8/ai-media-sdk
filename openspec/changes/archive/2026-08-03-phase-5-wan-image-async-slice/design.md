...
So the **async skeleton is complete and video-validated**. The Aliyun adapter already has a shared `getTask(taskId)` (`GET /tasks/{task_id}` + status mapping `PENDING`→`pending`, `PROCESSING`/`RUNNING`→`running`, `SUCCEEDED`→`succeeded`, `FAILED`→`failed`, `CANCELED`→`cancelled`, `UNKNOWN`→`failed`) built in Phase 4. The registry has `wan2.7-image-pro` / `wan2.7-image` / `z-image-turbo` as `wan-image` family entries; only the first two are account-supported for async submission, while `z-image-turbo` remains generation-only.
