...
#### Scenario: Volcengine Ark T2I and I2I perform real sync calls through the transport

- **WHEN** the Volcengine Ark adapter `generate` or `edit` is invoked for a registered Seedream model with valid configuration and an injected transport
...
- **THEN** Azure, Alibaba, and Volcengine Provider packages SHALL depend on the core package and development tooling only, with no `openai`, DashScope, or Volcengine Ark runtime SDK
