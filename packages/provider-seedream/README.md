...
[AI Media SDK](../ai-media-sdk/README.md) 的火山引擎方舟 Doubao-Seedream 图像生成 Provider。基于原生 `fetch` 调用方舟 OpenAI 兼容图像 API（同步），不依赖平台 SDK。
...
```bash
npm install @ai-media/sdk @ai-media/provider-seedream
```
...
通过 `createSeedreamProvider(config)` 传入配置对象：
...
Seedream 专属字段放在 `providerOptions.seedream` 命名空间下（类型 `SeedreamImageProviderOptions`）：
