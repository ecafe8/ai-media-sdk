...
describe("volcengine provider", () => {
  test("factory retains the injected transport and binds a known model", () => {
    ...
    const provider = createVolcengineProvider(VOLCENGINE_CONFIG, { transport });

    expect(provider.transport).toBe(transport);
    expect(provider.providerId).toBe("volcengine");
    const model = provider.image(PRO);
    expect(model.modelId).toBe(PRO);
    expect(model.providerId).toBe("volcengine");
  ...
  test("unknown model id is rejected with UNKNOWN_MODEL", () => {
    ...
    expect(error.message).toMatch(/Unknown Volcengine Ark model id/);
  });
  ...
  test("listModels projects every registry entry including the alias pair", () => {
    ...
    expect(models.every((m) => m.providerId === "volcengine")).toBe(true);
